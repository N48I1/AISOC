...
export async function mispSearchIocs(iocs: IocBundle): Promise<MispEnrichment> {
  ...
  // Two attempts with 1-second delay between them
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (status !== 200) {
        console.warn(`[MISP] HTTP ${status} (attempt ${attempt}): ${text.slice(0, 200)}`);
      ...
      console.warn(`[MISP] error (attempt ${attempt}): ${err?.message || err}`);
      if (attempt < 2) { await sleep(1000); continue; }
      return emptyEnrichment(false);
    }
  }
